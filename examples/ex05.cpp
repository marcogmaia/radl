...
struct location_t {
  ...
  [[nodiscard]] auto get_xy() const { return std::tuple{x, y}; }
};
...
struct map_t {
  ...
  inline int at(const int &x, const int &y) { return x + (y * width); }
...
struct navigator {
  ...
  // near it.
  static bool is_goal(location_t &pos, location_t &goal) { return pos == goal; }
