...
struct Location {
  ...
  auto get_xy() const { return std::tuple{x, y}; }
};
...
struct navigator {
  ...
  static float get_distance_estimate(Location &pos, Location &goal) {
  ...
  // near it.
  static bool is_goal(Location &pos, Location &goal) { return pos == goal; }
  ...
  static bool get_successors(Location pos, std::vector<Location> &successors) {
    ...
    for (int off_x = -1; off_x <= 1; ++off_x) {
      for (int off_y = -1; off_y <= 1; ++off_y) {
        ...
        auto sucessor = Location{px + off_x, py + off_y};
  ...
  static float get_cost(Location &position, Location &successor) {
    return 1.0f;
  }
  ...
  static bool is_same_state(Location &lhs, Location &rhs) { return lhs == rhs; }
  ...
  static int get_x(const Location &loc) { return loc.x; }
  static int get_y(const Location &loc) { return loc.y; }
  static Location get_xy(const int &x, const int &y) { return Location{x, y}; }
  static bool is_walkable(const Location &loc) {
...
// calls
class FovAssist final : public IFov {
public:
  bool is_blocked(int x, int y) override { return !map.walkable[map.at(x, y)]; }
...
void draw_map() {
  ...
  static const auto darkened_wall = apply_colored_light(wall_color, dim_light);
  static const auto not_seen_wall = apply_colored_light(wall_color, dark_light);
...
void tick(double duration_secs) {
  ...
  static astar_path_t<Location> path;
  ...
  if (tick_time >= tick_duration) {
    ...
    if (dude_position == destination) {
      ...
      if (walkable && get_mouse_button_state(MOUSE_BUTTON_LEFT)) {
        ...
        // for display only
        path = path_find<navigator, Location>(dude_position,
                                              Location{terminal_x, terminal_y},
                                              path_finder_limit_calcs);
      }
    } else if (!path.steps.empty()) {
      // Follow the breadcrumbs!
      Location next_step = path.steps.front();
  ...
  if (path.success) {
    ...
    for (auto step : path.steps) {
      ...
      if (dude_position == destination) {
        highlight = {177,
                     color_t::lerp(colors::DARK_GREEN, colors::LIGHTEST_GREEN,
                                   lerp_amount),
                     BLACK};
      } else {
        highlight = {
            177,
            color_t::lerp(colors::DARK_RED, colors::LIGHTEST_RED, lerp_amount),
            BLACK};
      }
