...
void tick(double duration_secs) {
  ...
  get_vterm(LOG_LAYER).print(1, 3, "Even more...", LIGHT_GREEN, DARKEST_GREEN);
  get_vterm(LOG_LAYER).print(1, 4, "... goes here", LIGHT_GREEN, DARKEST_GREEN);

  get_vterm(LAYER_OVERLAY).clear();
  constexpr auto bgcolor = BLANK;
  get_vterm(LAYER_OVERLAY)
      .set_char(11, 10, vchar_t{17, LIGHT_GREEN, bgcolor}); // Draw a left arrow
  get_vterm(LAYER_OVERLAY)
...
int main() {
  ...
  gui->add_layer(LAYER_OVERLAY, 0, 32, 1024 - 160, 768 - 32, "8x8", resize_main,
