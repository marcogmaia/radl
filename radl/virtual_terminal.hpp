...
public:
  ...
  virtual_terminal(const std::string &fontt, const int x = 0, const int y = 0,
      ...
      : m_font_tag(fontt), m_offset_x(x), m_offset_y(y),
   ...
   * @return size_t
   */
  inline size_t at(int x, int y) noexcept { return x + y * term_width; }
   ...
   * @return std::string
   */
  inline std::string get_font_tag() noexcept { return m_font->texture_tag; }
  ...
  inline void box(const color_t &fg = colors::White,
                  const color_t &bg = colors::NONE, bool double_lines = false) {
   ...
   */
  inline void set_tint(const color_t &color) { m_tint = color; }
   ...
   * @param alpha
   */
  inline void set_alpha(uint8_t alpha) { m_tint.a = alpha; }
