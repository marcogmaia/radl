...
/**
 * @brief Base type for retained-mode GUI controls.
 *
 */
struct gui_control_t {
    virtual void render(virtual_terminal* vterm) = 0;
    virtual bool mouse_in_control(const int tx, const int ty) {
        return false;
    }
...
struct gui_border_box_t : public gui_control_t {
    gui_border_box_t(const bool double_lines, const color_t fg,
                     const color_t bg)
        : is_double(double_lines)
        , foreground(fg)
        , background(bg) {}
...
struct gui_radiobuttons_t : public gui_control_t {
    gui_radiobuttons_t(const int X, const int Y, const std::string heading,
        ...
        for(const radio& r : options) {
            if(width < static_cast<int>(r.label.size()))
                width = static_cast<int>(r.label.size());
            if(r.checked)
        ...
        on_mouse_up = [](gui_control_t* ctrl, int tx, int ty) {
            ...
            if(me->click_started) {
                ...
                if(option_number >= 0
                   && option_number <= static_cast<int>(me->options.size())) {
...
struct gui_hbar_t : public gui_control_t {
    gui_hbar_t(const int X, const int Y, const int W, const int MIN,
               const int MAX, const int VAL, const color_t FULL_START,
...
struct gui_vbar_t : public gui_control_t {
    gui_vbar_t(const int X, const int Y, const int H, const int MIN,
               const int MAX, const int VAL, const color_t FULL_START,
               const color_t FULL_END, const color_t EMPTY_START,
               const color_t EMPTY_END, const color_t TEXT_COL,
               std::string PREFIX = "")
...
struct gui_listbox_t : public gui_control_t {
    gui_listbox_t(const int X, const int Y, const int VAL,
                  std::vector<list_item> options, std::string label,
                  const color_t label_fg, const color_t label_bg,
                  const color_t ITEM_FG, const color_t ITEM_BG,
        ...
        for(const list_item& item : items) {
            if(w < static_cast<int>(item.label.size()))
                w = static_cast<int>(item.label.size());
        }
        ...
        on_mouse_up = [](gui_control_t* ctrl, int tx, int ty) {
            ...
            if(me->click_started) {
                const int option_number = (ty - me->y) - 1;
                if(option_number >= 0
                   && option_number <= static_cast<int>(me->items.size())) {
