 ...
 *for details)
 *
 *   Example contributed by @seanpringle and reviewed by Max (@moliad) and Ramon
 *Santamaria (@raysan5)
 *
 *   Copyright (c) 2020-2021 @seanpringle, Max (@moliad) and Ramon Santamaria
 *(@raysan5)
 *
 ********************************************************************************************/
...
int main(void) {
    ...
    SetConfigFlags(FLAG_MSAA_4X_HINT);  // Enable Multi Sampling Anti Aliasing
                                        // 4x (if available)
    ...
    float x = 0.0f, y = 0.0f,
          z = 0.0f;  // Used for various 3D coordinate & vector ops
    ...
    Matrix *rotations = (Matrix *)RL_MALLOC(MAX_INSTANCES * sizeof(Matrix));
    // Per-frame rotation animation of instances
    Matrix *rotationsInc = (Matrix *)RL_MALLOC(MAX_INSTANCES * sizeof(Matrix));
    // Locations of instances
    Matrix *translations = (Matrix *)RL_MALLOC(MAX_INSTANCES * sizeof(Matrix));
    ...
    Matrix *transforms = (Matrix *)RL_MALLOC(
        MAX_INSTANCES
        * sizeof(Matrix));  // Pre-multiplied transformations passed to rlgl

    Shader shader = LoadShader(
        TextFormat("resources/shaders/glsl%i/base_lighting_instanced.vs",
                   GLSL_VERSION),
    ...
    while(!WindowShouldClose())  // Detect window close button or ESC key
    {
        ...
        if(IsKeyDown(KEY_EQUAL))
            speed = (speed <= (fps * 0.25f)) ? (fps * 0.25f) : (speed * 0.95f);
        ...
        for(int i = 0; i < MAX_INSTANCES; i++) {
            ...
            loop = (float)((framesCounter
                            + (int)(((float)(i % groups) / groups) * speed))
                           % speed)
                   / speed;
            ...
            y = (sinf(loop * PI * 2)) * amp
                * ((1 - variance)
                   + (variance * (float)(i % (groups * 10)) / (groups * 10)));

            // Clamp to floor
            y = (y < 0) ? 0.0f : y;

            transforms[i]
                = MatrixMultiply(transforms[i], MatrixTranslate(0.0f, y, 0.0f));
        }
        ...
        DrawText(TextFormat(": %d = %f loops/sec", speed, ((float)fps / speed)),
                 160, textPositionY, 10, BLACK);
